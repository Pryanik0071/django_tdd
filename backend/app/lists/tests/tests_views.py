...
class HomePageTest(TestCase):
    ...
    def test_home_page_uses_item_form(self):
        """Домашняя страница использует форму для элемента"""
        response = self.client.get("/")
...
class ListViewTest(TestCase):
    ...
    def test_displays_item_form(self):
        """Отображение формы для элемента"""
        list_ = List.objects.create()
        response = self.client.get(f'/lists/{list_.id}/')
        self.assertIsInstance(response.context['form'], ItemForm)
    ...
    def test_for_invalid_input_nothing_saved_to_db(self):
        """Недопустимый ввод и ничего не сохраняется в бд"""
        self.post_invalid_input()
        self.assertEqual(Item.objects.count(), 0)

    def test_for_invalid_input_renders_list_template(self):
        """Недопустимый ввод - отображается шаблон списка"""
        response = self.post_invalid_input()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "list.html")

    def test_for_invalid_input_passes_form_to_template(self):
        """Недопустимый ввод - форма передается в шаблон"""
        response = self.post_invalid_input()
        self.assertIsInstance(response.context['form'], ItemForm)

    def test_for_invalid_input_shows_error_on_page(self):
        """Недопустимый ввод - на странице показывается ошибка"""
        response = self.post_invalid_input()
...
class NewListTest(TestCase):
    ...
    def test_for_invalid_input_home_template(self):
    ...
    def test_validation_errors_are_shown_on_home_page(self):
        """Ошибка валидации выводятся на домашней странице"""
        response = self.client.post("/lists/new", data={"text": ""})
        self.assertContains(response, escape(EMPTY_ITEM_ERROR))
